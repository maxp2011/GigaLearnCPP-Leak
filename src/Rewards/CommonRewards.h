...
class PopResetReward : public Reward {
	public:
		// How many steps after gaining the reset the flip can be used.
		// At 8-tick skip (~15 steps/sec), 15 steps ~= 1 second.
		int   maxStepsAfterReset;

		// Ball must go upward after the pop (UU/s)
		float minBallVelZ;

		// Ball must not fly too far sideways/forward - keeps it controlled (UU/s)
		float maxBallVelXY;

		// Ball must not rocket straight up - keeps it in reachable range (UU/s)
		float maxBallVelZ;

		PopResetReward(
			int   maxStepsAfterReset = 15,
			float minBallVelZ        = 100.f,
			float maxBallVelXY       = 800.f,
			float maxBallVelZ        = 900.f
		) : maxStepsAfterReset(maxStepsAfterReset),
		    minBallVelZ(minBallVelZ),
		    maxBallVelXY(maxBallVelXY),
		    maxBallVelZ(maxBallVelZ) {
		}

		virtual void Reset(const GameState& state) override {
			stepsSinceReset.clear();
			chainCount.clear();
		}
		...
		virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
			...
			// Landing breaks the chain and kills the window
			if (player.isOnGround) {
				if (chainCount[id] > 0)
					std::cout << "PopResetReward: chain of " << chainCount[id] << " broken by landing" << std::endl;
				stepsSinceReset[id] = -1;
				chainCount[id]      = 0;
				return 0.f;
			}
			...
			// Stage 1: flip reset just gained -> open the window
			if (!prevHadFlipReset && nowHasFlipReset) {
				stepsSinceReset[id] = 0;
			}

			// Tick the window if active
			int& steps = stepsSinceReset[id];
			if (steps >= 0)
				steps++;

			// Window expired without a valid pop - break the chain
			if (steps > maxStepsAfterReset) {
				if (chainCount[id] > 0)
					std::cout << "PopResetReward: chain of " << chainCount[id] << " broken by timeout" << std::endl;
				steps          = -1;
				chainCount[id] = 0;
				return 0.f;
			}

			// Stage 2: inside the window, flip was consumed AND ball was touched
			bool flipConsumed = prevHadFlipReset && !nowHasFlipReset;
			bool touchedBall  = player.ballTouchedStep;

			if (steps >= 0 && flipConsumed && touchedBall) {
				steps = -1; // Close the window regardless of outcome

				Vec   ballVel = state.ball.vel;
				float velZ    = ballVel.z;
				float velXY   = Vec(ballVel.x, ballVel.y, 0).Length();

				bool goesUp     = velZ  >  minBallVelZ;
				bool notTooFlat = velXY <  maxBallVelXY;
				bool notTooHigh = velZ  <  maxBallVelZ;

				if (goesUp && notTooFlat && notTooHigh) {
					// Increment chain and compute reward: 3^(chainCount - 1)
					...
					std::cout << "PopResetReward: chain x" << chain
					          << " -> " << reward
					          << " (velZ=" << velZ << " velXY=" << velXY << ")" << std::endl;
					return reward;
				} else {
					// Bad pop - break the chain
					if (chainCount[id] > 0)
						std::cout << "PopResetReward: chain of " << chainCount[id] << " broken by bad pop (velZ=" << velZ << " velXY=" << velXY << ")" << std::endl;
					chainCount[id] = 0;
				}
			}
	...
	private:
		// -1 = no active window, >= 0 = steps since flip reset was gained
		std::map<uint32_t, int> stepsSinceReset;

		// How many consecutive successful pops this aerial
...
public:
	...
	public:
		...
		virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override
		{
			...
			// --- Facing ball ---
			Vec normPosDiff = Normalize(posDiff);
			float facingBall = Dot(player.rotMat.forward, normPosDiff);
			...
			float offensiveReward = cosine_similarity(state.ball.pos - player.pos, attacc - player.pos);
